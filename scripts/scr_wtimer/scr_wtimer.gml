// wTimer by Distant Illusions
// A timer system for scheduling and delaying function executions.
 ...
 * file, You can obtain one at https://mozilla.org/MPL/2.0/. */
 ...
 #macro __WTIMER_VERSION "2.2.0"

 show_debug_message("wTimer v" + __WTIMER_VERSION + " by Distant Illusions");
 ...
 function timer(_callback, _duration, _repeat = false)
 {
 ...
 /// @param {mixed} arguments...[OPTIONAL] Arguments that can be used with the callback function. You can have a maximum of 16. Optional.
 /// @returns {real} The ID corresponding to the timer.
 function timer_ms(_callback, _duration, _repeat = false)
 {
     var _arguments = undefined;
     if (argument_count > 3)
     {
 ...
 /// @param {mixed} arguments...[OPTIONAL] Arguments that can be used with the callback function. You can have a maximum of 16. Optional.
 /// @returns {real} The ID corresponding to the timer.
 function timer_trigger(_callback, _trigger, _repeat = false)
 {
     var _arguments = undefined;
     if (argument_count > 3)
     {
 ...
 /// @desc Returns either the remaining duration of the given timer, or the trigger function if the timer uses that.
 /// @param {real} timer The index of the timer to get the remaining duration of.
 /// @returns {real or function} Either the remaining duration or the trigger function of the given timer. Returns -1 if the timer doesn't exist.
 ...
 function timer_get_type(_timer)
 {
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
         var _result = global.__timer_list[| i];
 ...
 function timer_set_paused(_timer)
 {
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
         var _result = global.__timer_list[| i];
 ...
 function timer_get_paused(_timer)
 {
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
         var _result = global.__timer_list[| i];
 ...
 function timer_set_multiplier(_timer, _multiplier)
 {
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
         var _result = global.__timer_list[| i];
         if (_result.index == _timer)
         {
             _result.multiplier = _multiplier;
             return true;
         }
     }
     return false;
 ...
 function timer_set_multiplier_object(_id, _multiplier)
 {
     var _return = 0;
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
 ...
 function timer_get_multiplier(_timer)
 {
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
         var _result = global.__timer_list[| i];
 ...
 function timer_change(_timer, _duration, _change_repeat_time = false)
 {
     for (var i = 0; i < ds_list_size(global.__timer_list); i++)
     {
         var _result = global.__timer_list[| i];
         if (_result.index == _timer)
         {
             if (_change_repeat_time)
                 _result.initial_duration = _duration;
             else
                 _result.value = _duration;
             return true;
         }
     }
     return false;
 ...
 function wait(_duration, _offset = 0)
 {
     gml_pragma("forceinline");
     return ((current_frames + _offset + ((argument_count > 1) ? argument[1] : 0)) % _duration) < 1;
 }
 
 /// @func wait_ms(duration, offset[OPTIONAL])
 /// @desc Always returns false for a specified interval, after which will return true for one frame. Repeats endlessly, and ignores the timer tick rate.
 /// @param {real} duration The interval between true returns in milliseconds.
 /// @param {real} offset[OPTIONAL] Adds an additional offset to the timer in milliseconds. Optional.
 /// @returns {bool} True or false depending on the interval.
 function wait_ms(_duration, _offset = 0)
 {
     gml_pragma("forceinline");
     var _frame_time = (delta_time / 1000) / _duration;
     var _current_total_time = (current_time / 1000 + _offset) / _duration;
     return floor(_current_total_time) > floor(_current_total_time - _frame_time);
 }